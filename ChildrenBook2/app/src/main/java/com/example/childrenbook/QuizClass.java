...
public class QuizClass extends ArrayAdapter<Quiz> {
    public QuizClass(@NonNull Context context, ArrayList<Quiz> arrayList) {
        super(context, 0,arrayList);
    }
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        Quiz mcq = getItem(position);
        Quiz mcq2 = getItem(position);
        convertView= LayoutInflater.from(getContext()).inflate(R.layout.ql,null,true);
